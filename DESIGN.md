...
RawBlock is a brutalist design system for experimental portfolios and art sites. Black-on-white, thick borders, no rounded corners, no shadows.
...
Base unit: 8px. Usage is intentionally irregular; treat the scale as a starting point.
...
- **radius-none** (0px): All elements, no exceptions.

## Elevation

No shadows. Hierarchy comes from border weight and scale contrast.

- **shadow-none**: None. All elements.
- Borders: border-thin (1px), border-thick (3px), border-heavy (5px).
...
1. **Do** use thick borders (3-5px) as the primary visual organizer instead of shadows.
2. **Don't** round any corners.
3. **Do** use full color inversions (black/white) for hover and active states.
4. **Don't** use opacity for disabled states; use lighter borders and grey fills.
5. **Do** use Archivo Black at large sizes (48-64px).
6. **Don't** use blue (#0000FF) for anything but hyperlinks.
7. **Do** allow spacing irregularity and asymmetric layouts.
8. **Do** use uppercase + tracking for buttons and labels.
9. **Don't** add decorative images, icons, or illustrations.
10. **Don't** polish; if something looks too "designed," strip it back.
